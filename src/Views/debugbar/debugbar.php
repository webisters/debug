...
if (!isset($options['info_contents'])) {
    $options['info_contents'] = '<p>★ <a href="https://webisters.com" target="_blank">webisters.com</a></p>';
}
...
?>
<!-- Webisters Debugbar start -->
<style>
    <?= file_get_contents(__DIR__ . '/../assets/prism-webisters.css') ?>
</style>
<style>
    <?php
...
<div id="debugbar" class="webisters-debug" dir="ltr">
    <div class="panels">
        <div class="panel info-collection">
            ...
            <div class="contents">
                <div class="collector-default">
                    <p>Running<?=
                        class_exists('Webisters')
                            ? ' <a href="https://webisters.com" target="_blank" class="webisters-link">Webisters</a> ' . Debugger::roundVersion(Webisters::VERSION)
...
?>
        ...
        <?php foreach ($collections as $collection): ?>
            <?php if ($collection->hasCollectors()): ?>
                <div class="panel <?= $collection->getSafeName() ?>-collection">
                    ...
                    <header>
                        <div class="title">
                            <span class="title-icon"><?= $renderBadgeIcon($collection->getSafeName(), $collection->getName()) ?></span>
    ...
    <div class="toolbar">
        <div class="icon">
            <img src="data:image/png;base64,<?= base64_encode((string) file_get_contents($iconPath)) ?>" alt="W" width="48" height="52">
        </div>
                ...
                <?php if ($collection->hasCollectors()): ?>
                    <button class="collection" id="<?= $collection->getSafeName() ?>-collection"
                        title="<?= $collection->getName() ?> Collection">
                            <?= $renderBadgeIcon($collection->getSafeName(), $collection->getName()) ?>
